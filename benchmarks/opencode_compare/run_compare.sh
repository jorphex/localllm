...
COMPARE_LABEL="${COMPARE_LABEL:-qwen-vs-omnicoder}"
...
COMPARE_CANDIDATES="${COMPARE_CANDIDATES:-qwen-3.5-abl omnicoder-9b}"
COMPARE_RESTORE_PRESET="${COMPARE_RESTORE_PRESET:-omnicoder-9b}"
...
run_candidate() {
  ...
  trap 'compare_stop_candidate "${pid}"; restore_main_service' EXIT
  ...
  trap 'restore_main_service' EXIT
}
...
write_run_manifest() {
  jq -cn \
    ...
    --argjson omnicoder "${OMNICODER_SPEC}" \
    --argjson nemotron "${NEMOTRON_SPEC}" \
    '{
      label:$label,
      ...
      model_dir:$model_dir,
      candidates:[$qwen, $omnicoder, $nemotron]
